...
%%%%%%%%%%%%%%%%%%%%%%%%% Step 2: Generate fluctuation data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % Initialize the vector for saving observed data: the statistics
    ...
    parfor j = 1 : nmc
      ...
      [tmp1, tmp2] = countsizeBDtree3(a, a_dt, mu_vec,chkt);
      Nt(j,1) = tmp1;
      Xt(j,1) = tmp2;
...
save(strcat('~/ABC/simulation/data/constP_varyA/delta2/Y',num2str(seed),'_L1.mat'),'Nt','Xt','obs_X','MOM','duration');
end
delete(POOL);
...
%%%%%%%%% Step 1: Model specifications for the ABC estimator %%%%%%%%%%%%%%%%%%%
  load('~/ABC/simulation/data/constP_varyA/delta2/Y1.mat')
theta_sigma = [20 20];        % sd of prior distribution
p_range = [-9 -3];            % range for the three parameters 
dt_range = [-1 0];

% model specifications
model_spec.num_training_theta = [100; 10];    % Number of settings to train and refine GP model 
...
model_spec.trans_step = [0.1 0.1];         % The transition steps of the MCMC algorithm 
model_spec.theta_old = [log10(0.8) -6];    % t_d = 4  % The initial value of the three parameters

model_spec.init = 'init_final.mat';
...
for seed = 1:100                             % change seed range to run less cases, each takes around 40 minutes.
    ...
    theta_mu = [0 log10(MOM)];   % Initial value corresponding to MOM estimator`
    tStart = tic;
