...
function [Sample,runningTime,initTime] = ABC_mu2(theta_mu, theta_sigma, a_dt, p_range, obs_X, model_spec)
    ...
    if model_spec.init == 0
      startTime = tic;
      [theta_list, X] = getIniX2(a, dt_range,p_range,chkt,S0, num_rep,time_update, Z0, constraint);
      ...
      if model_spec.saveSample == 1
        save('init_2.mat','theta_list', 'theta_list_s', "X", "initTime");
      end
    else
    ...
    [theta_new,p_range_new] = get_theta2(trans_step,theta_old,a_dt,p_range,constraint);
...
%             sample = randmvn(Muj,Sigmaj,M,eps)';
            ...
            if Eps <= ksi
              ...
              [Theta_new, X_delta] = getIniX2(a,a_dt,p_range_new,chkt,delta, num_rep,time_update,Z0,constraint);
