...
function [Sample, runningTime, initTime] = ABC_mu1(theta_mu, theta_sigma, a_dt, p_range, obs_X, model_spec)
    ...
    if model_spec.init == 0
      startTime = tic;
      [theta_list, X] = getIniX1(a, a_dt,p_range,chkt,S0, num_rep,time_update, Z0);
    ...
    [theta_new,p_range_new] = get_theta1(trans_step,theta_old,p_range);
...
%             sample = randmvn(Muj,Sigmaj,M,eps)';
            ...
            if Eps <= ksi
              ...
              [Theta_new, X_delta] = getIniX1(a,a_dt,p_range_new,chkt,delta, num_rep,time_update,Z0);
    ...
    %if model_spec.saveSample == 1
    %  save('init_1_final.mat','theta_list', 'theta_list_s','X');
    %end
    %close(gcf)
