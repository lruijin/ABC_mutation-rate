...
function [Sample] = ABC_mu3(theta_mu, theta_sigma, dt_range, p_range, obs_X, model_spec)
    ...
    elseif length(dt_range) == 2
      ...
      error("The range of delta could be either 1 or 2.")
    end
    ...
    if model_spec.init == 0
      ...
      save('init_final2.mat','theta_list', 'theta_list_s', "X", "initTime");
    else
    ...
    [theta_new,dt_range_new,p_range_new] = get_theta2(trans_step,theta_old,dt_range,p_range,constraint);
...
%             sample = randmvn(Muj,Sigmaj,M,eps)';
            ...
            if Eps <= ksi
              ...
              [Theta_new, X_delta] = getIniX2(a,dt_range_new,p_range_new,chkt,delta, num_rep,time_update,L,constraint);
    ...
    runningTime = toc(startTime);
     ...
     save('sample2.mat','runningTime','Sample');
         close(gcf)