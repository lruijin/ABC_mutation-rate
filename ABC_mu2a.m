...
function [Sample, runningTime, initTime] = ABC_mu2a(theta_mu, theta_sigma, dt_range, p_range, obs_X, model_spec)
    ...
    if model_spec.init == 0
      startTime = tic;
      [theta_list, X] = getIniX2a(a, dt_range,p_range,chkt,S0, num_rep,time_update, Z0, constraint);
      ...
      if model_spec.saveSample == 1
        save('post/init_2a.mat','theta_list', 'theta_list_s', "X", "initTime",'param','model','param_sd','model_sd');
      end
    else
    ...
    [theta_new,dt_range_new,p_range_new] = get_theta2a(trans_step,theta_old,dt_range,p_range,constraint);
...
%             sample = randmvn(Muj,Sigmaj,M,eps)';
            ...
            if Eps <= ksi
              ...
              [Theta_new, X_delta] = getIniX2a(a,dt_range_new,p_range_new,chkt,delta, num_rep,time_update,Z0,constraint);
